...
pub fn wasm_std_funs() -> Stack {
    ...
    [I32, I64, F32, F64].iter().for_each(|t| {
        vec!["add", "mul", "sub", "eq", "ne"].drain(..).for_each(|name| {
            stack.push(name.to_owned(), Type::WasmFn(vec![fun_type!([t.clone() t.clone()](t.clone()))]), true, false).unwrap();
        });
    });

    [I32, I64].iter().for_each(|t| {
        vec!["gt_s", "gt_u", "ge_s", "ge_u", "lt_s", "lt_u", "le_s", "le_u",
             "div_u", "div_s", "rem_u", "rem_s", "and", "or", "xor",
             "shl", "shr_u", "shr_s", "rotl", "rotr"].drain(..).for_each(|name| {
            stack.push(name.to_owned(), Type::WasmFn(vec![fun_type!([t.clone() t.clone()](t.clone()))]), true, false).unwrap();
        });
    });

    [F32, F64].iter().for_each(|t| {
        vec!["gt", "ge", "lt", "le", "div", "min", "max", "copysign"].drain(..).for_each(|name| {
            stack.push(name.to_owned(), Type::WasmFn(vec![fun_type!([t.clone() t.clone()](t.clone()))]), true, false).unwrap();
        });
    });

    [I32, I64].iter().for_each(|t| {
        vec!["clz", "ctz", "popcnt", "eqz", "extend_8s", "extend_16s"].drain(..).for_each(|name| {
            stack.push(name.to_owned(), Type::WasmFn(vec![fun_type!([t.clone()](t.clone()))]), true, false).unwrap();
        });
    });

    [F32, F64].iter().for_each(|t| {
        vec!["abs", "neg", "sqrt", "ceil", "floor", "trunc", "nearest"].drain(..).for_each(|name| {
            stack.push(name.to_owned(), Type::WasmFn(vec![fun_type!([t.clone()](t.clone()))]), true, false).unwrap();
        });
    });
