...
pub fn parse_ext(parser: &mut Parser) -> Result<(String, Vec<ExtDef>), WasminError> {
    if let Some(mod_name) = parser.parse_word() {
        ...
        if let Some('{') = parser.curr_char() {
            ...
            Err(werr_syntax!(
                "expected '{' after module name, got EOF",
                parser.pos()
            ))
        }
    } else {
        Err(werr_syntax!("expected module name, got EOF", parser.pos()))
    }
}
...
fn parse_defs(parser: &mut Parser, mod_name: &str) -> Result<Vec<ExtDef>, WasminError> {
    ...
    parser
        .stack_mut()
        .push_namespace(mod_name.to_owned(), defs)
        .map_err(|msg| werr_syntax!(msg, mod_pos))?;
    Ok(ext_defs)
}
